...
from nile.constants import get_all_contracts, CONTRACTS_DIRECTORY
...
async def _test_command(contracts):
  if len(contracts) == 0:
    print(f"🤖 Running all test Cairo contracts in the {CONTRACTS_DIRECTORY} directory")
    for path in get_all_contracts(".test.cairo"):
      await run_test_contract(path)
  else:
    for path in contracts:
      await run_test_contract(path)


async def run_test_contract(path):
